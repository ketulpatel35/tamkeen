# -*- coding:utf-8 -*-
# from PyQt4.uic.Compiler.qobjectcreator import _QtGuiWrapper
# from distutils.command.config import config
from pytz import timezone, utc
# from twisted.python.rebuild import latestFunction
...
class HrPayslip(models.Model):
    ...
    @api.model
    def _book_holiday_hours(self, contract, presence_policy,
                            ot_policy, attendances, holiday_obj,
    ...
    def get_worked_day_lines(self, contract_ids, date_from, date_to):
        ...
        for contract in self.env['hr.contract'].browse(contract_ids):
            ...
            if contract.schedule_template_id:
                contract_rest_days = \
                    contract.schedule_template_id.get_rest_days()
            ...
            for day in range(0, nb_of_days):
                ...
                if scheduled_hours == 0 and dtDateTime.weekday() not in \
                    ...
                    if contract.schedule_template_id:
                        scheduled_hours = \
                            contract.schedule_template_id.get_hours_by_weekday(
                                dtDateTime.weekday())
                ...
                if working_hours_on_day:
                    ...
                    if public_holiday:
                        _hours, push_lsd = self._book_holiday_hours(
                            contract, presence_policy, ot_policy, attendances,
                ...
                leave_type, leave_hours = self.leaves_list_get_hours(
                    ...
                    contract.id, contract.schedule_template_id,
    ...
    def get_payslip_lines(self, contract_ids, payslip_id):
        ...
        get_contracts = self.get_contract(
            payslip.employee_id, payslip.date_from, payslip.date_to)
        get_old_contracts = []
        for get_old_contract in get_contracts:
            if get_old_contract not in contracts.ids:
                get_old_contracts = get_old_contract
        get_old_contracts_rec = \
            self.env['hr.contract'].browse(get_old_contracts)
        dict_contract_wage = {}
        for contract_r in get_old_contracts_rec:
            if contract_r.date_end and payslip.date_from:
                contract_end_date = \
                    datetime.strptime(contract_r.date_end, OE_DATEFORMAT)
                paslip_date_from = \
                    datetime.strptime(payslip.date_from, OE_DATEFORMAT)
                if contract_end_date >= paslip_date_from:
                    contract_diff = \
                        abs(contract_end_date - paslip_date_from).days + 1
                    dict_contract_wage.\
                        update({'days': contract_diff,
                                'wage': contract_r.wage})
        for contract in contracts:
            ...
            # localdict_old = {'result': None, 'result_qty': 1.0,
            #                  'result_rate': 100,
            #                  'worked_days': worked_days,
            #                  'inputs': inputs, 'employee': employee,
            #                  'contract': get_old_contracts_rec[0],
            #                  'payslip': payslip,
            ...
            for rule in sorted_rules:
                ...
                if rule.satisfy_condition(localdict) and rule.id not in \
                    ...
                    # print ':::::::::::', amount
                    # localdict_old['result'] = None
                    # localdict_old['result_qty'] = 1.0
                    # localdict_old['result_rate'] = 100
                    # # if amount and qty and rate:
                    # # amount_c, latest_contract_wage = 0.0, 0.0
                    # payslip_date_from =\
                    #     datetime.strptime(payslip.date_from, OE_DATEFORMAT)
                    # payslip_date_to =\
                    #     datetime.strptime(payslip.date_to, OE_DATEFORMAT)
                    # payslip_current_days = \
                    #     abs(payslip_date_from - payslip_date_to).days
                    # # for d_contract in dict_contract_wage:
                    # latest_contract_days =\
                    #     payslip_current_days - dict_contract_wage.get('days')
                    # amount_old_contract, old_qty, old_rate =\
                    #     rule.compute_rule(localdict_old)
                    # amount_c = (amount_old_contract *
                    #             dict_contract_wage.get('days')) / 30
                    # latest_contract_wage =
                    #  (amount * latest_contract_days) / 30
                    # amount = amount_c + latest_contract_wage
                    # old_tot_rule =\
                    #     amount_old_contract * old_qty * old_rate / 100.0
                    # localdict_old[rule.code] = old_tot_rule
                    # old_previous_amount =\
                    #     rule.code in localdict_old\
                    #     and localdict_old[rule.code] or 0.0
                    # localdict_old = \
                    #     _sum_salary_rule_category(localdict_old,
                    #                               rule.category_id,
                    #                               old_tot_rule -
                    #                               old_previous_amount)
                    # Modified code for for 3 days
