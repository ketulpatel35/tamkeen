...
class Meh_Action(odoo.addons.web.controllers.main.Action):
    ...
    def load(self, action_id):
        ...
        user_groups = frozenset(req.session.model('res.users').read(
            [req.session._uid], ['groups_id'], req.context)[0]['groups_id'])
        visible_menu_ids = []
        if base_action:
            ctx = {}
            ...
            if action_type == 'ir.actions.act_window' and action[
                    0].get('id', False):
                ...
                value_ids = ir_values_obj.search(
                    [
                        ...
                        ('value',
                         '=',
                         'ir.actions.act_window,{0}'.format(
                             action[0]['id']))],
                    context=req.context)
                menu_ids = [
                    menu['res_id'] for menu in ir_values_obj.read(
                        value_ids, ['res_id'], req.context)]
                ...
                for menu_id in menu_ids:
                    cmenu_id = menu_id
                    while True:
                        parent_menu = ir_menu_obj.read(
                            cmenu_id, ['parent_id'], context=req.context) or []
                        parent_menu_id = parent_menu['parent_id'][
                            0] if parent_menu['parent_id'] else False
            ...
            if ((action and action[0].get('groups_id', []) == []) and
                ...
                value = odoo.addons.web.controllers.main.clean_action(action[
                                                                      0])
        return value
