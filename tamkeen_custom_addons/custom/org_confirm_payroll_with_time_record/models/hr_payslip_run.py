...
class HrPayslipRun(models.Model):
    ...
    def confirm_payslip_run(self):
        time_record_line_obj = self.env['sheet.time.recording.line']
        for rec in self:
            if not rec.slip_ids:
            ...
            time_record_line_rec = time_record_line_obj.search([('date', '>=',
                                                  rec.date_start),
                                         ('date', '<=', rec.date_end),
                                         ('payroll_locked', '=', True),
                                         ('payroll_locked_date', '!=', False)])
            for line in time_record_line_rec:
                line.write({'affected_payroll': True})
        return True
